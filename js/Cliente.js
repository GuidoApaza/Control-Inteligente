$(document).ready(function() {
  ...
  function buscar_cliente(consulta){
    ...
    $.post('../controlador/ClienteController.php',{consulta,funcion},(response)=>{
      ...
      clientes.forEach(cliente => {
        template+=`$ 
          <tr clientId="${cliente.id}" clientNombre="${cliente.nombre}" clientApellido="${cliente.apellido}" clientDNI="${cliente.dni}"clientfecha="${cliente.fecha}">
  ...
  $(document).on('keyup',"#buscar-cliente",function(){
    ...
    let fecha_actualizacion=$('#fecha-cliente').val();
    console.log(nombre_cliente,apellido_cliente,dni_cliente,fecha_actualizacion);
    if (edit==false) {
   ...
   $.post('../controlador/ClienteController.php',{id_editado,nombre_cliente,apellido_cliente,dni_cliente,fecha_actualizacion,funcion},(response)=>{
      if (response=='add') {
        $('#add').hide('slow');
        $('#add').show(1000);
        $('#add').hide(2000);
  ...
  $(document).on('click','.editar-cliente',(e)=>{   
    ...
    const fecha=$(elemento).attr('clientfecha'); 
